...
import com.omneagate.activity.dialog.CardInspectionDialog_remove;
...
public class CardInspectionActivity extends BaseActivity implements View.OnClickListener, View.OnFocusChangeListener {
    ...
    EditText remarks, firstText, secondText, thirdText;
    InspectionRemoveDialog inspectionRemoveDialog;
    CardInspectionDialog_remove cardInspectionDialogRemove;
    ...
    public static boolean cardInspectionDialogValidation = false;
    ...
    private void findView() {
        firstText = (EditText) findViewById(R.id.cardInspFirstText);
        secondText = (EditText) findViewById(R.id.cardInspSecondText);
        thirdText = (EditText) findViewById(R.id.cardInspThirdText);
        watcher = new TextWatcher() {
            ...
            public void afterTextChanged(Editable editable) {
                if (editable == firstText.getEditableText() && firstText.getText().length() == 2 && firstText.getText().length() != 0) {
                    secondText.requestFocus();
                } else if (editable == secondText.getEditableText() && secondText.getText().length() != 0) {
...
//        mBtCancel.setOnClickListener(this);
    ...
    private void loadViewData() {
        ...
        mTvTitle.setText(getResources().getString(R.string.title_verification_inspection));
        total.setText(": " + String.valueOf(Util.findingCriteriaDto.getCardInspection().size()) + " " + getResources().getString(R.string.inspection));
        listSize = Util.findingCriteriaDto.getCardInspection().size();
        loadProductSpinner();
    ...
    private void loadProductSpinner() {
        ...
        commoditySpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parentView, View selectedItemView, int position, long id) {
                ...
                double qty = FPSDBHelper.getInstance(CardInspectionActivity.this).getStockOfSpecificProduct(productId);
                cardInspectionDialogRemove = new CardInspectionDialog_remove(com.omneagate.activity.CardInspectionActivity.this, qty, productNameStr);
                cardInspectionDialogRemove.show();
                cardInspectionDialogRemove.setCanceledOnTouchOutside(false);
                remarks.setText("");
            }
...
//        stockInspectionDtoList = FPSDBHelper.getInstance(this).getAllStockInspectionData();
        ...
        total.setText(": " + String.valueOf(Util.findingCriteriaDto.getCardInspection().size()) + " " + getResources().getString(R.string.inspection));
        loadTableValues(Util.findingCriteriaDto.getCardInspection());
    }
...
//            linearBg.setBackgroundResource(R.drawable.spinner_background);
    ...
    private void loadTableValues(List<CardInspectionDto> value) {
        try {
            LinearLayout transactionLayout = (LinearLayout) findViewById(R.id.listView_linearLayout_stock_status);
            transactionLayout.removeAllViews();
            LayoutInflater lin = LayoutInflater.from(CardInspectionActivity.this);
            int sno = 1;
    ...
    private View returnView(LayoutInflater entitle, final int sno, final CardInspectionDto cardInspectionDto, final int position) {
...
//        ImageView viewRecord = (ImageView) convertView.findViewById(R.id.img_explore);
        mTvSno.setText("" + sno);
//        final String productName = FPSDBHelper.getInstance(this).getProductName(cardInspectionDto.getCommodity());
        mTvType.setText(cardInspectionDto.getCardNumber());
        remove.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                inspectionRemoveDialog = new InspectionRemoveDialog(com.omneagate.activity.CardInspectionActivity.this, position, "", "CardInspectionActivity");
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.btn_submit:
                ...
                boolean added = addcardInspection();
                if (added) {
                    finishStockInspection();
                }
                break;
...
//                break;
    ...
    public void onBackPressed() {
        ...
        if (!(commoditySpinner.getSelectedItem() == null)) {
            if (!cardInspectionDialogValidation) {
                finishStockInspection();
            } else {
                unsavedStockInspectionDialog = new UnsavedStockInspectionDialog(com.omneagate.activity.CardInspectionActivity.this, getResources().getString(R.string.unsavedcardInspection));
    ...
    public void finishStockInspection() {
        Intent intent = new Intent(CardInspectionActivity.this, InspectionFindingActivity.class);
        startActivity(intent);
        finish();
    }

    private boolean addcardInspection() {
        Log.e(TAG, "existingStock..." + existingStock);
        if (FPSDBConstants.checkedittext(firstText) || FPSDBConstants.checkedittext(secondText) || FPSDBConstants.checkedittext(thirdText)) {
            Toast.makeText(CardInspectionActivity.this, R.string.enter_ration, Toast.LENGTH_SHORT).show();
            return false;
        } else if (commoditySpinner.getSelectedItem() == null) {
            Toast.makeText(CardInspectionActivity.this, R.string.sel_commodity, Toast.LENGTH_SHORT).show();
            return false;
        } else if (remarks.getText().toString().trim().equalsIgnoreCase("")) {
            Toast.makeText(CardInspectionActivity.this, R.string.enter_remarks, Toast.LENGTH_SHORT).show();
            return false;
        } else if (!cardInspectionDialogValidation) {
            Toast.makeText(CardInspectionActivity.this, R.string.enter_physical_stock, Toast.LENGTH_SHORT).show();
            return false;
        } else if (containsProductId(Util.findingCriteriaDto.getCardInspection(), Long.valueOf(productId))) {
            Toast.makeText(CardInspectionActivity.this, R.string.stock_already_inspected, Toast.LENGTH_SHORT).show();
            return false;
        } else if (FPSDBHelper.getInstance(this).checkRationCardNumber(getrationcardnumber()) == 0) {
            Toast.makeText(CardInspectionActivity.this, R.string.ration_exists, Toast.LENGTH_SHORT).show();
            return false;
        } else {
//            InspectionReportDto inspectionReportDto = FPSDBHelper.getInstance(CardInspectionActivity.this).getLastInsertedReportClientId();
            CardInspectionDto cardInspectionDto = new CardInspectionDto();
            cardInspectionDto.setCommodity(Long.valueOf(productId));
            cardInspectionDto.setCommodityIssuedasperPos(systemStock);
            cardInspectionDto.setCommodityIssuedasperCard(existingStock);
            cardInspectionDto.setVariance(stockVariance);
            cardInspectionDto.setRemarks(remarks.getText().toString());
            cardInspectionDto.setCardNumber(getrationcardnumber());
//            cardInspectionDto.setTransactionId(Util.getInspectionStockTransactionId(this));
//                    cardInspectionDto.setClientReportId(inspectionReportDto.getClientId());
//                    cardInspectionDtoList.add(stockInspectionDto);
            systemStock = 0.0;
            existingStock = 0.0;
            stockVariance = 0.0;
            cardInspectionDialogValidation = false;
//            FindingCriteriaDto findingCriteria = new FindingCriteriaDto();
//            findingCriteria.setCriteria("Stock");
//            findingCriteria.setStockInspection(stockInspectionDto);
            Util.findingCriteriaDto.getCardInspection().add(cardInspectionDto);
            listSize = Util.findingCriteriaDto.getCardInspection().size();
            loadStockInspectionList();
            loadProductSpinner();
            cleartext();
    ...
    public boolean containsProductId(List<CardInspectionDto> list, long id) {
        for (CardInspectionDto cardInspectionDto : list) {
            if (cardInspectionDto.getCommodity() == id && cardInspectionDto.getCardNumber().equals(getrationcardnumber())) {
                return true;
            }
        }
    ...
    private void removeAllDialogs() {
        ...
        try {
            if ((cardInspectionDialogRemove != null) && cardInspectionDialogRemove.isShowing()) {
                cardInspectionDialogRemove.dismiss();
            }
        } catch (final Exception e) {
        } finally {
            cardInspectionDialogRemove = null;
        }
        try {
