...
import com.omneagate.activity.dialog.BestFingerForMemberSuccessDialog;
...
public class TgTenFingerFpsMemberRegistrationActivity extends BaseActivity implements MFS100Event {
    //String fingerPrintAadhar = "",
    ...
    String TAG = "TgTenFingerRegistrationActivity";
    String Activityname;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tg_ten_finger_registration);
        setPopUpPage();
        networkConnection = new NetworkConnection(this);
        httpConnection = new HttpClientWrapper();
        if (GlobalAppState.language.equals("te"))
            ((TextView) findViewById(R.id.top_textView)).setText("FPS సభ్యులు- ఉత్తమ వేలు డిటెక్షన్");
        else
            ((TextView) findViewById(R.id.top_textView)).setText("FPS MEMBER - BEST FINGER DETECTION");
    ...
    private void setUpInitialScreen() {
        ...
        updateDateTime();
        //fingerPrintAadhar = getIntent().getStringExtra("uid");
        Activityname = getIntent().getStringExtra("ActivityName");
        memberName = getIntent().getExtras().getString("memberName");
        Uid = getIntent().getExtras().getString("uid");

        ((TextView) findViewById(R.id.holdername)).setText("       " + memberName + " - " + Uid);

        rationCardlayout=(LinearLayout)findViewById(R.id.fps_is);
...
//                        process();
                            ...
                            if (selectedPosition.equalsIgnoreCase("0")) {
                                if (!(isoFeatureSet_01.length > 0)) {
                                    ...
                                    if (isoFeatureSet_01.length > 0) {
                                      ...
                                      /*  MediaPlayer mPlayer = MediaPlayer.create(TgTenFingerRegistrationActivity.this, R.raw.beeptg);
                                        mPlayer.start();*/
                                        beep.start();
                                        if ((isoFeatureSet_01.length > 0) && (isoFeatureSet_02.length > 0) && (isoFeatureSet_03.length > 0) && (isoFeatureSet_04.length > 0) && (isoFeatureSet_05.length > 0)
                                                && (isoFeatureSet_06.length > 0) && (isoFeatureSet_07.length > 0) && (isoFeatureSet_08.length > 0) && (isoFeatureSet_09.length > 0) && (isoFeatureSet_10.length > 0)) {
                                            ((Button) findViewById(R.id.btnSubmit)).setText(R.string.Submit);

                                        }

                                    }
                                } else {
                                    Toast.makeText(TgTenFingerFpsMemberRegistrationActivity.this, R.string.scanned_already, Toast.LENGTH_SHORT).show();
                                }
        ...
        ArrayAdapter<String> fingerAdapt = new ArrayAdapter<>(TgTenFingerFpsMemberRegistrationActivity.this, android.R.layout.simple_spinner_item, finger);
        ...
        try {
            ...
            copyAssetToDir(TgTenFingerFpsMemberRegistrationActivity.this.getAssets(), "uidai_auth_prod.cer", destDir);
        } catch (Exception e) {
    ...
    private void errorNavigation(String messages) {
        Toast.makeText(TgTenFingerFpsMemberRegistrationActivity.this, messages, Toast.LENGTH_SHORT).show();
    }

    @Override
    public void onBackPressed() {
        if (Activityname.equalsIgnoreCase("TgSalesEntryActivity")) {
            Intent backIntent1 = new Intent(TgTenFingerFpsMemberRegistrationActivity.this, TgSalesEntryActivity.class);
            startActivity(backIntent1);
            finish();

        } else {
            Intent backIntent = new Intent(TgTenFingerFpsMemberRegistrationActivity.this, TgFpsMembersActivity.class);
    ...
    public void process() {
        try {
            progressBar = new CustomProgressDialog(TgTenFingerFpsMemberRegistrationActivity.this);
            progressBar.setCanceledOnTouchOutside(false);
            if (NetworkUtil.getConnectivityStatus(TgTenFingerFpsMemberRegistrationActivity.this) == 0) {
                Util.messageBar(TgTenFingerFpsMemberRegistrationActivity.this, getString(R.string.no_connectivity));
            } else {
                progressBar.show();
                sendKYCRequest();
            }
    ...
    class GetBFDDetails extends AsyncTask<String, String, GeneralResponse> {
        @Override
        protected void onPreExecute() {
            super.onPreExecute();
            rationCardDetailDialog = new RationCardDetailDialog(TgTenFingerFpsMemberRegistrationActivity.this, getString(R.string.authenticatingFingerPrint));
        ...
        @Override
        protected void onPostExecute(GeneralResponse s) {
            super.onPostExecute(s);
            rationCardDetailDialog.cancel();

            if (s != null && s.getRespMsgCode() != null) {
                Log.e("THE RESPONSE MESSAGE", s.getRespMsgCode());


                if (s.getRespMsgCode().equalsIgnoreCase("0")) {
                    new BestFingerForMemberSuccessDialog(TgTenFingerFpsMemberRegistrationActivity.this, "TgTenFingerFpsMemberRegistrationActivity").show();
                }
            }


//            Toast.makeText(TgTenFingerRegistrationActivity.this, s.getRespMsgCode(), Toast.LENGTH_SHORT).show();
//            setValues(s);
        }
    }


    private GeneralResponse sendRequestForBFD() {
        ...
        try {
           ...
           // GregorianCalendar gc = new GregorianCalendar();
            ...
            String pidXml = buildRbdXml(dateString, encodedBioMetricInfo_01, encodedBioMetricInfo_02,
                    encodedBioMetricInfo_03, encodedBioMetricInfo_04, encodedBioMetricInfo_05,
                    encodedBioMetricInfo_06, encodedBioMetricInfo_07, encodedBioMetricInfo_08,
                    encodedBioMetricInfo_09, encodedBioMetricInfo_10);
            ...
            inputMap.put("rationCard", "0");
            inputMap.put("memberId", "0");
            inputMap.put("typeId", "0");
            ...
            generalResponse = XMLUtil.postUidBestFingers(inputMap);
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    TgGenericErrorDialog tgGenericErrorDialog =new TgGenericErrorDialog(TgTenFingerFpsMemberRegistrationActivity.this,""+e.getMessage());
    ...
    private void sendKYCRequest() {
        try {
            ...
            com.android.volley.RequestQueue queue = Volley.newRequestQueue(TgTenFingerFpsMemberRegistrationActivity.this, stack);
            String rbdXml;
            final String encodedBioMetricInfo_01 = Base64.encodeToString(isoFeatureSet_01, Base64.DEFAULT);
            final String encodedBioMetricInfo_02 = Base64.encodeToString(isoFeatureSet_02, Base64.DEFAULT);
            final String encodedBioMetricInfo_03 = Base64.encodeToString(isoFeatureSet_03, Base64.DEFAULT);
            ...
            rbdXml = buildRbdXml(dateString, encodedBioMetricInfo_01, encodedBioMetricInfo_02,
                    encodedBioMetricInfo_03, encodedBioMetricInfo_04, encodedBioMetricInfo_05,
                    encodedBioMetricInfo_06, encodedBioMetricInfo_07, encodedBioMetricInfo_08,
                    encodedBioMetricInfo_09, encodedBioMetricInfo_10);
            ...
            JsonObjectRequest req = new JsonObjectRequest(uriString, jsonBody, new com.android.volley.Response.Listener<JSONObject>() {
                public void onResponse(JSONObject response) {
                    Toast.makeText(TgTenFingerFpsMemberRegistrationActivity.this, "success", Toast.LENGTH_SHORT).show();
                    ...
                    try {
                        ...
                        if (status.equalsIgnoreCase("success")) {
                            ...
                            if (getIntent().getStringExtra("MemberType").equalsIgnoreCase("0")) {
                                long benefId = FPSDBHelper.getInstance(TgTenFingerFpsMemberRegistrationActivity.this).getBeneficiaryIdFromUid(Uid);
                                long benefMemberId = FPSDBHelper.getInstance(TgTenFingerFpsMemberRegistrationActivity.this).getBeneficiaryMemberIdFromUid(Uid);
...
//                                bfdDetailsDto.setBeneficiaryMemberId(benefMemberId);
                    ...
                    new com.android.volley.Response.ErrorListener() {
                        ...
                        public void onErrorResponse(VolleyError error) {
                            ...
                            Util.messageBar(TgTenFingerFpsMemberRegistrationActivity.this, getString(R.string.connectionError));
                        }
                    });
            queue.add(req);
        } catch (Exception e) {
    ...
    private void sendBfdDetailToServer(BFDDetailDto bfdDetailDto) {
        ...
        try {
            Util.LoggingQueue(TgTenFingerFpsMemberRegistrationActivity.this, "Info", "Bfd task started");
            NetworkConnection network = new NetworkConnection(TgTenFingerFpsMemberRegistrationActivity.this);
            ...
            Util.LoggingQueue(TgTenFingerFpsMemberRegistrationActivity.this, "bfd details service exception..", e.getMessage());
        }
    }
    ...
    private void bfdRegSubmissionResponse(Bundle message) {
        ...
        if (bfdDetailDto.getStatusCode() == 0) {
            boolean inserted = FPSDBHelper.getInstance(TgTenFingerFpsMemberRegistrationActivity.this).insertBfdDetails(bfdDetailDto);
//            FPSDBHelper.getInstance(AddProxyDetailsActivity.this).updateProxyDetailsSyncStatus(proxyDetailsDto.getBeneficiary().getId());
            if (inserted) {
                ...
                if (getIntent().getStringExtra("MemberType").equalsIgnoreCase("1")) {
                    String rcNo = getIntent().getStringExtra("RcNumber");
                    new AddProxySuccess(TgTenFingerFpsMemberRegistrationActivity.this, rcNo).show();

                } else {
                    new BfdRegSuccessDialog(TgTenFingerFpsMemberRegistrationActivity.this).show();
                }
            }
        } else {
            String messageData = "";
            try {
                messageData = Util.messageSelection(FPSDBHelper.getInstance(TgTenFingerFpsMemberRegistrationActivity.this).retrieveLanguageTable(bfdDetailDto.getStatusCode()));
            } catch (Exception e) {
            ...
            if (StringUtils.isEmpty(messageData))
                messageData = "Bfd sync failed";
            Toast.makeText(TgTenFingerFpsMemberRegistrationActivity.this, messageData, Toast.LENGTH_SHORT).show();
            Util.LoggingQueue(TgTenFingerFpsMemberRegistrationActivity.this, "Statistics service", "Error syncing bfd detail..." + messageData);
        }
    }
        ...
        try {
            Thread trd = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (ret != 0) {
                        Toast.makeText(TgTenFingerFpsMemberRegistrationActivity.this, Util.mfs100.GetErrorMsg(ret), Toast.LENGTH_SHORT).show();
                    } else {
                        ...
                        Log.e("sample bitmap app", "bitmap value..." + bitmapImg);
                    }
                }
            });
            trd.run();
    ...
    public void OnDeviceAttached(int vid, int pid, boolean hasPermission) {
        int ret = 0;
        if (!hasPermission) {
            Toast.makeText(TgTenFingerFpsMemberRegistrationActivity.this, "Permission denied", Toast.LENGTH_SHORT).show();
            return;
        }
        if (vid == 1204 || vid == 11279) {
            if (pid == 34323) {
                ...
                if (ret != 0) {
                    ...
                    Toast.makeText(TgTenFingerFpsMemberRegistrationActivity.this, "Loadfirmware success", Toast.LENGTH_SHORT).show();
                }
            } else if (pid == 4101) {
...
//                    Toast.makeText(TenFingerRegistrationActivity.this, "Uninit Success", Toast.LENGTH_SHORT).show();
    ...
    private class LogOutTimerTask extends TimerTask {
        ...
        public void run() {
            ...
            Intent i = new Intent(TgTenFingerFpsMemberRegistrationActivity.this, TgLoginActivity.class);
