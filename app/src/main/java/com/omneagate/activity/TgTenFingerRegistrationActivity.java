...
public class TgTenFingerRegistrationActivity extends BaseActivity implements MFS100Event {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (GlobalAppState.language.equals("te"))
            ((TextView) findViewById(R.id.top_textView)).setText("సభ్యుడు - ఉత్తమ వేలు డిటెక్షన్");
        else
            ((TextView) findViewById(R.id.top_textView)).setText("MEMBER - BEST FINGER DETECTION");
                        ...
                        if (Enroll_Template.length > 0) {
                            if (selectedPosition.equalsIgnoreCase("0")) {
                                if (!(isoFeatureSet_01.length > 0)) {
                                    ...
                                    Toast.makeText(TgTenFingerRegistrationActivity.this, R.string.scanned_already, Toast.LENGTH_SHORT).show();
                                }
...
//                                        ((Button)findViewById(R.id.btnSubmit)).setText(R.string.Submit);
        ...
        ArrayAdapter<String> fingerAdapt = new ArrayAdapter<>(TgTenFingerRegistrationActivity.this, android.R.layout.simple_spinner_item, finger);
        ...
        try {
            ...
            copyAssetToDir(TgTenFingerRegistrationActivity.this.getAssets(), "uidai_auth_prod.cer", destDir);
        } catch (Exception e) {
    ...
    private void errorNavigation(String messages) {
        Toast.makeText(TgTenFingerRegistrationActivity.this, messages, Toast.LENGTH_SHORT).show();
    }

    @Override
    public void onBackPressed() {
        if (Activityname.equalsIgnoreCase("TgSalesEntryActivity")) {
            Intent backIntent1 = new Intent(TgTenFingerRegistrationActivity.this, TgSalesEntryActivity.class);
            startActivity(backIntent1);
            finish();

        } else {
            Intent backIntent = new Intent(TgTenFingerRegistrationActivity.this, TgFpsMembersActivity.class);
    ...
    public void process() {
        try {
            progressBar = new CustomProgressDialog(TgTenFingerRegistrationActivity.this);
            progressBar.setCanceledOnTouchOutside(false);
            if (NetworkUtil.getConnectivityStatus(TgTenFingerRegistrationActivity.this) == 0) {
                Util.messageBar(TgTenFingerRegistrationActivity.this, getString(R.string.no_connectivity));
            } else {
                progressBar.show();
                sendKYCRequest();
            }
    ...
    class GetBFDDetails extends AsyncTask<String, String, GeneralResponse> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            rationCardDetailDialog = new RationCardDetailDialog(TgTenFingerRegistrationActivity.this, getString(R.string.authenticatingFingerPrint));
        ...
        protected void onPostExecute(GeneralResponse s) {
            ...
            if (s.getRespMsgCode().equalsIgnoreCase("0")) {
                new BestFingerForMemberSuccessDialog(TgTenFingerRegistrationActivity.this,"TgTenFingerRegistrationActivity").show();
            }
            }
...
//            setValues(s);
    ...
    private GeneralResponse sendRequestForBFD() {
        ...
        try {
            ...
            inputMap.put("typeId", XMLUtil.RC_MEMBER_ID);
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    TgGenericErrorDialog tgGenericErrorDialog =new TgGenericErrorDialog(TgTenFingerRegistrationActivity.this,""+e.getMessage());
    ...
    private void sendKYCRequest() {
        try {
            ...
            com.android.volley.RequestQueue queue = Volley.newRequestQueue(TgTenFingerRegistrationActivity.this, stack);
            String rbdXml;
            final String encodedBioMetricInfo_01 = Base64.encodeToString(isoFeatureSet_01, Base64.DEFAULT);
            final String encodedBioMetricInfo_02 = Base64.encodeToString(isoFeatureSet_02, Base64.DEFAULT);
            final String encodedBioMetricInfo_03 = Base64.encodeToString(isoFeatureSet_03, Base64.DEFAULT);
            ...
            JsonObjectRequest req = new JsonObjectRequest(uriString, jsonBody, new com.android.volley.Response.Listener<JSONObject>() {
                public void onResponse(JSONObject response) {
                    Toast.makeText(TgTenFingerRegistrationActivity.this, "success", Toast.LENGTH_SHORT).show();
                    ...
                    try {
                        ...
                        if (status.equalsIgnoreCase("success")) {
                            ...
                            if (getIntent().getStringExtra("MemberType").equalsIgnoreCase("0")) {
                                long benefId = FPSDBHelper.getInstance(TgTenFingerRegistrationActivity.this).getBeneficiaryIdFromUid(fingerPrintAadhar);
                                long benefMemberId = FPSDBHelper.getInstance(TgTenFingerRegistrationActivity.this).getBeneficiaryMemberIdFromUid(fingerPrintAadhar);
...
//                                bfdDetailsDto.setBeneficiaryMemberId(benefMemberId);
                    ...
                    new com.android.volley.Response.ErrorListener() {
                        ...
                        public void onErrorResponse(VolleyError error) {
                            ...
                            Util.messageBar(TgTenFingerRegistrationActivity.this, getString(R.string.connectionError));
                        }
                    });
            queue.add(req);
        } catch (Exception e) {
    ...
    private void sendBfdDetailToServer(BFDDetailDto bfdDetailDto) {
        ...
        try {
            Util.LoggingQueue(TgTenFingerRegistrationActivity.this, "Info", "Bfd task started");
            NetworkConnection network = new NetworkConnection(TgTenFingerRegistrationActivity.this);
            ...
            Util.LoggingQueue(TgTenFingerRegistrationActivity.this, "bfd details service exception..", e.getMessage());
        }
    }
    ...
    private void bfdRegSubmissionResponse(Bundle message) {
        ...
        if (bfdDetailDto.getStatusCode() == 0) {
            boolean inserted = FPSDBHelper.getInstance(TgTenFingerRegistrationActivity.this).insertBfdDetails(bfdDetailDto);
//            FPSDBHelper.getInstance(AddProxyDetailsActivity.this).updateProxyDetailsSyncStatus(proxyDetailsDto.getBeneficiary().getId());
            if (inserted) {
                ...
                if (getIntent().getStringExtra("MemberType").equalsIgnoreCase("1")) {
                    String rcNo = getIntent().getStringExtra("RcNumber");
                    new AddProxySuccess(TgTenFingerRegistrationActivity.this, rcNo).show();

                } else {
                    new BfdRegSuccessDialog(TgTenFingerRegistrationActivity.this).show();
                }
            }
        } else {
            String messageData = "";
            try {
                messageData = Util.messageSelection(FPSDBHelper.getInstance(TgTenFingerRegistrationActivity.this).retrieveLanguageTable(bfdDetailDto.getStatusCode()));
            } catch (Exception e) {
            ...
            if (StringUtils.isEmpty(messageData))
                messageData = "Bfd sync failed";
            Toast.makeText(TgTenFingerRegistrationActivity.this, messageData, Toast.LENGTH_SHORT).show();
            Util.LoggingQueue(TgTenFingerRegistrationActivity.this, "Statistics service", "Error syncing bfd detail..." + messageData);
        }
    }
        ...
        try {
            Thread trd = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    if (ret != 0) {
                        Toast.makeText(TgTenFingerRegistrationActivity.this, Util.mfs100.GetErrorMsg(ret), Toast.LENGTH_SHORT).show();
                    } else {
    ...
    public void OnDeviceAttached(int vid, int pid, boolean hasPermission) {
        int ret = 0;
        if (!hasPermission) {
            Toast.makeText(TgTenFingerRegistrationActivity.this, "Permission denied", Toast.LENGTH_SHORT).show();
            return;
        }
        if (vid == 1204 || vid == 11279) {
            if (pid == 34323) {
                ...
                if (ret != 0) {
                    ...
                    Toast.makeText(TgTenFingerRegistrationActivity.this, "Loadfirmware success", Toast.LENGTH_SHORT).show();
                }
            } else if (pid == 4101) {
...
//                    Toast.makeText(TenFingerRegistrationActivity.this, "Uninit Success", Toast.LENGTH_SHORT).show();
    ...
    private class LogOutTimerTask extends TimerTask {
        ...
        public void run() {
            ...
            Intent i = new Intent(TgTenFingerRegistrationActivity.this, TgLoginActivity.class);
