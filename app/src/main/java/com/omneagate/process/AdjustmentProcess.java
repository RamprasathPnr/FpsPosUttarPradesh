...
public class AdjustmentProcess implements BaseSchedulerService, Serializable {

    Context globalContext;
    BufferedReader in = null;
    String TAG = "AdjustmentProcess";
    String serverUrl = "";
    ...
    private void startProcess() {
        boolean sessionInvalid = AdjustmentTask();
        if(sessionInvalid) {
    ...
    public boolean AdjustmentTask() {
        boolean unauthorized = false;
        long primaryId = -1;
        try {
            NetworkConnection network = new NetworkConnection(globalContext);
            if (network.isNetworkAvailable()) {
                List<POSStockAdjustmentDto> posAcknowledge = FPSDBHelper.getInstance(globalContext).stockAdjustmentDataToServer();
                Util.LoggingQueue(globalContext, TAG, "UpdateAdjustment() POSStockAdjustmentDto = " + posAcknowledge.toString());
                for (POSStockAdjustmentDto acknowledgedData : posAcknowledge) {
                    Util.LoggingQueue(globalContext, TAG, "syncAdjustmentsToServer() POSStockAdjustmentDto = " + acknowledgedData.toString());
                    String url = serverUrl + "/posstockadjustment/fps/acknowledge";
                    URI website = new URI(url);
                    String adjustmentData = new Gson().toJson(acknowledgedData);
                    // inserting request into local db
                    primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(adjustmentData, "AdjustmentService");
                    StringEntity entity = new StringEntity(adjustmentData, HTTP.UTF_8);
                    ...
                    if (response != null && response.getStatusLine() != null && response.getStatusLine().getStatusCode() == HttpURLConnection.HTTP_OK) {
                        ...
                        if ((responseData != null) && (!responseData.contains("<html>"))) {
                            ...
                            POSStockAdjustmentDto posData =  gson.fromJson(responseData, POSStockAdjustmentDto.class);
                            String messageData = "";
                            String status = "";
                            Log.e(TAG, "response:" + posData.toString());
                            if (posData.getStatusCode() == 0 ||posData.getStatusCode() == 12008 ||posData.getStatusCode() == 12009) {
                                status = "Success";
                                FPSDBHelper.getInstance(globalContext).adjustmentUpdate(posData);
                            }
                            else {
                                status = "Failure";
                                try {
                                    messageData = Util.messageSelection(FPSDBHelper.getInstance(globalContext).retrieveLanguageTable(posData.getStatusCode()));
                                    if (messageData == null) {
                ...
                // inserting request into local db
                primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "AdjustmentService");
                // update response into local db
                if (primaryId != -1) {
        ...
        if (network.isNetworkAvailable()) {
            ...
            try {
                ...
                if(loginDto != null) {
                    ...
                    // inserting request into local db
                    sessionPrimaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(loginDetails, "Session_request_AdjustmentService");
...
//                        SessionId.getInstance().setFpsId(loginResponse.getUserDetailDto().getFpsStore().getId());
    ...
    private void insertLoginCredentialException() {
        long id = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "Session_request_AdjustmentService");
