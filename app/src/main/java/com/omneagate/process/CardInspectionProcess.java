...
public class CardInspectionProcess implements BaseSchedulerService, Serializable {

    Context globalContext;
    BufferedReader in = null;
    List<CardInspectionDto> listCardInspectionDto;
    String TAG = "CardInspectionProcess";
    String serverUrl = "";
    ...
    private void startProcess() {
        boolean sessionInvalid = CardInspectionTask();
        if(sessionInvalid) {
    ...
    public boolean CardInspectionTask() {
        boolean unauthorized = false;
        long primaryId = -1;
        try {
            NetworkConnection network = new NetworkConnection(globalContext);
            if (network.isNetworkAvailable()) {
                listCardInspectionDto = new ArrayList<CardInspectionDto>();
                listCardInspectionDto = FPSDBHelper.getInstance(globalContext).getAllCardVerificationImages();
                for (CardInspectionDto cardInspectionDto : listCardInspectionDto) {
                    cardInspectionDto.setClientReportId(cardInspectionDto.getId());//local db card table primary key
                    cardInspectionDto.setId(null);
                    String url = serverUrl + "/inspection/app/card";
                    URI website = new URI(url);
                    String cardInspectionData = new Gson().toJson(cardInspectionDto);
                    // inserting request into local db
                    primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(cardInspectionData, "CardInspectionService");
                    StringEntity entity = new StringEntity(cardInspectionData, HTTP.UTF_8);
                    ...
                    if (response != null && response.getStatusLine() != null && response.getStatusLine().getStatusCode() == HttpURLConnection.HTTP_OK) {
                        ...
                        if ((responseData != null) && (!responseData.contains("<html>"))) {
                            ...
                            CardInspectionDto cardInspectionDto2 = gson.fromJson(responseData, CardInspectionDto.class);
                            String messageData = "";
                            String status = "";
                            Log.e(TAG, "response:" + cardInspectionDto2.toString());
                            if (cardInspectionDto2 != null && cardInspectionDto2.getStatusCode() == 0) {
                                status = "Success";
                                Log.e(TAG, "ImageResponseCardTableId" + cardInspectionDto2.getClientReportId());
                                FPSDBHelper.getInstance(globalContext).updateCardVerificationTable(cardInspectionDto2.getClientReportId());//It is client primary key card table
                                Log.e(TAG, "OfflineImageCardService Updating status of Report Image T for transfered " + cardInspectionDto2.getClientReportId());
                            }
                            else {
                                status = "Failure";
                                try {
                                    messageData = Util.messageSelection(FPSDBHelper.getInstance(globalContext).retrieveLanguageTable(cardInspectionDto2.getStatusCode()));
                                    if (messageData == null) {
                ...
                // inserting request into local db
                primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "CardInspectionService");
                // update response into local db
                if (primaryId != -1) {
        ...
        if (network.isNetworkAvailable()) {
            ...
            try {
                ...
                if(loginDto != null) {
                    ...
                    // inserting request into local db
                    sessionPrimaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(loginDetails, "Session_request_CardInspectionService");
...
//                        SessionId.getInstance().setFpsId(loginResponse.getUserDetailDto().getFpsStore().getId());
    ...
    private void insertLoginCredentialException() {
        long id = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "Session_request_CardInspectionService");
