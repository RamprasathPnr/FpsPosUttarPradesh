...
public class CloseSaleProcess implements BaseSchedulerService, Serializable {

    Context globalContext;
    BufferedReader in = null;
    List<CloseSaleTransactionDto> closeSaleList;
    String TAG = "CloseSaleProcess";
    String serverUrl = "";
    ...
    private void startProcess() {
        boolean sessionInvalid = CloseSaleTask();
        if(sessionInvalid) {
    ...
    public boolean CloseSaleTask() {
        ...
        try {
            ...
            if (network.isNetworkAvailable()) {
                ...
                closeSaleList = FPSDBHelper.getInstance(globalContext).getAllCloseSaleForSync();
                for (CloseSaleTransactionDto bill : closeSaleList) {
                    bill.setDeviceId(Settings.Secure.getString(globalContext.getContentResolver(), Settings.Secure.ANDROID_ID).toUpperCase());
                    String url = serverUrl + "/closeofsale/save";
                    URI website = new URI(url);
                    String closeSaleData = new Gson().toJson(bill);
                    // inserting request into local db
                    primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(closeSaleData, "CloseSaleService");
                    StringEntity entity = new StringEntity(closeSaleData, HTTP.UTF_8);
                    ...
                    if (response != null && response.getStatusLine() != null && response.getStatusLine().getStatusCode() == HttpURLConnection.HTTP_OK) {
                        ...
                        if ((responseData != null) && (!responseData.contains("<html>"))) {
                            ...
                            CloseSaleTransactionDto closeSaleTransactionDto = gson.fromJson(responseData, CloseSaleTransactionDto.class);
                            String messageData = "";
                            String status = "";
                            Log.e(TAG, "response:" + closeSaleTransactionDto.toString());
                            if (closeSaleTransactionDto != null && (closeSaleTransactionDto.getStatusCode() == 0 || closeSaleTransactionDto.getStatusCode() == 7001)) {
                                status = "Success";
                                FPSDBHelper.getInstance(globalContext).updateCloseDale(closeSaleTransactionDto);
                                Util.LoggingQueue(globalContext, TAG, "Updating status of close sale to status T for bill id " + closeSaleTransactionDto.getTransactionId());
                            }
                            else {
                                status = "Failure";
                                try {
                                    messageData = Util.messageSelection(FPSDBHelper.getInstance(globalContext).retrieveLanguageTable(closeSaleTransactionDto.getStatusCode()));
                                    if (messageData == null) {
                ...
                // inserting request into local db
                primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "CloseSaleService");
                // update response into local db
                if (primaryId != -1) {
        ...
        if (network.isNetworkAvailable()) {
            ...
            try {
                ...
                if(loginDto != null) {
                    ...
                    // inserting request into local db
                    sessionPrimaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(loginDetails, "Session_request_CloseSaleService");
...
//                        SessionId.getInstance().setFpsId(loginResponse.getUserDetailDto().getFpsStore().getId());
    ...
    private void insertLoginCredentialException() {
        long id = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "Session_request_CloseSaleService");
