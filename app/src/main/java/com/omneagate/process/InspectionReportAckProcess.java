...
public class InspectionReportAckProcess implements BaseSchedulerService, Serializable {

    Context globalContext;
    BufferedReader in = null;
    List<InspectionReportDto> inspectionReportList;
    String TAG = "InspectionReportAckProcess";
    String serverUrl = "";
    ...
    private void startProcess() {
        boolean sessionInvalid = InspectionReportAckTask();
        if(sessionInvalid) {
            String sessionId = getSessionFromServer();
            SessionId.getInstance().setSessionId(sessionId);
            if((SessionId.getInstance().getSessionId() != null) && (!SessionId.getInstance().getSessionId().equalsIgnoreCase(""))) {
                InspectionReportAckTask();
            }
        }
    }

    public boolean InspectionReportAckTask() {
        ...
        try {
            ...
            if (network.isNetworkAvailable()) {
                ...
                inspectionReportList = FPSDBHelper.getInstance(globalContext).getReportsAckUnsend();
                for (InspectionReportDto inspectionReportDtoRequest : inspectionReportList) {
                    String url = serverUrl + "/inspection/app/report/acknowledge";
                    URI website = new URI(url);
                    String inspectionReportAckData = new Gson().toJson(inspectionReportDtoRequest);
                    // inserting request into local db
                    primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(inspectionReportAckData, "InspectionReportAckService");
                    StringEntity entity = new StringEntity(inspectionReportAckData, HTTP.UTF_8);
                    ...
                    if (response != null && response.getStatusLine() != null && response.getStatusLine().getStatusCode() == HttpURLConnection.HTTP_OK) {
                        ...
                        if ((responseData != null) && (!responseData.contains("<html>"))) {
                            ...
                            InspectionReportDto inspectionReportDto = gson.fromJson(responseData, InspectionReportDto.class);
                            String messageData = "";
                            String status = "";
                            Log.e(TAG, "response:" + inspectionReportDto.toString());
                            if (inspectionReportDto != null && inspectionReportDto.getStatusCode() == 0 && inspectionReportDto.getId() != 0) {
                                status = "Success";
                                Log.e(TAG, "OfflineReportsAckResponse..."+inspectionReportDto.toString());
                                FPSDBHelper.getInstance(globalContext).updateFpsAckSyncStatus(inspectionReportDto.getClientId());
                                Log.e(TAG, "Updating Report Ack Table service " + inspectionReportDto.getId());
                            }
                            else if (inspectionReportDto != null && inspectionReportDto.getStatusCode() == 8808 && inspectionReportDto.getId() != 0) {
                                status = "Success";
                                Log.e(TAG, "OfflineReportsAckResponse..."+inspectionReportDto.toString());
                                FPSDBHelper.getInstance(globalContext).updateFpsAckSyncStatus(inspectionReportDto.getClientId());
                                Log.e(TAG, "Updating Report Ack Table service " + inspectionReportDto.getId());
                                ...
                                status = "Failure";
                                try {
                                    messageData = Util.messageSelection(FPSDBHelper.getInstance(globalContext).retrieveLanguageTable(inspectionReportDto.getStatusCode()));
                                    if (messageData == null) {
                ...
                // inserting request into local db
                primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "InspectionReportService");
                // update response into local db
                if (primaryId != -1) {
        ...
        if (network.isNetworkAvailable()) {
            ...
            try {
                ...
                if(loginDto != null) {
                    ...
                    // inserting request into local db
                    sessionPrimaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(loginDetails, "Session_request_InspectionReportAckService");
...
//                        SessionId.getInstance().setFpsId(loginResponse.getUserDetailDto().getFpsStore().getId());
    ...
    private void insertLoginCredentialException() {
        long id = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "Session_request_InspectionReportAckService");
