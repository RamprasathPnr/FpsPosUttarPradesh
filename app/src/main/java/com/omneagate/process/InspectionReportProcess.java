...
public class InspectionReportProcess implements BaseSchedulerService, Serializable {

    Context globalContext;
    BufferedReader in = null;
    List<InspectionReportDto> inspectionReportList;
    String TAG = "InspectionReportProcess";
    String serverUrl = "";
    ...
    private void startProcess() {
        boolean sessionInvalid = InspectionReportTask();
        if(sessionInvalid) {
            String sessionId = getSessionFromServer();
            SessionId.getInstance().setSessionId(sessionId);
            if((SessionId.getInstance().getSessionId() != null) && (!SessionId.getInstance().getSessionId().equalsIgnoreCase(""))) {
                InspectionReportTask();
            }
        }
    }

    public boolean InspectionReportTask() {
        ...
        try {
            ...
            if (network.isNetworkAvailable()) {
                ...
                inspectionReportList = FPSDBHelper.getInstance(globalContext).getReportsUnsend();
                for (InspectionReportDto inspectionReportDtoRequest : inspectionReportList) {
                    String url = serverUrl + "/inspection/app/savereport";
                    URI website = new URI(url);
                    String inspectionReportData = new Gson().toJson(inspectionReportDtoRequest);
                    // inserting request into local db
                    primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(inspectionReportData, "InspectionReportService");
                    StringEntity entity = new StringEntity(inspectionReportData, HTTP.UTF_8);
                    ...
                    if (response != null && response.getStatusLine() != null && response.getStatusLine().getStatusCode() == HttpURLConnection.HTTP_OK) {
                        ...
                        if ((responseData != null) && (!responseData.contains("<html>"))) {
                            ...
                            if (inspectionReportDto != null && inspectionReportDto.getStatusCode() == 0 && inspectionReportDto.getId() != 0) {
                                status = "Success";
                                Log.e(TAG, "OfflineReportsResponse..."+inspectionReportDto.toString());
                                FPSDBHelper.getInstance(globalContext).updateReportTransfered(inspectionReportDto);
                                Log.e(TAG, "Updating Report Table service " + inspectionReportDto.getId());
                            }
                            else if (inspectionReportDto != null && inspectionReportDto.getStatusCode() == 8805 && inspectionReportDto.getId() != 0) {
                                status = "Success";
                                Log.e(TAG, "OfflineReportsResponse..."+inspectionReportDto.toString());
                                FPSDBHelper.getInstance(globalContext).updateReportTransfered(inspectionReportDto);
                                Log.e(TAG, "Updating Report Table service " + inspectionReportDto.getId());
        ...
        if (network.isNetworkAvailable()) {
            ...
            try {
                ...
                if(loginDto != null) {
                    ...
                    // inserting request into local db
                    sessionPrimaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(loginDetails, "Session_request_InspectionReportService");
...
//                        SessionId.getInstance().setFpsId(loginResponse.getUserDetailDto().getFpsStore().getId());
    ...
    private void insertLoginCredentialException() {
        long id = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "Session_request_InspectionReportService");
