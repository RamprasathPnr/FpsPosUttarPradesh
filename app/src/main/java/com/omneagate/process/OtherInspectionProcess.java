...
public class OtherInspectionProcess implements BaseSchedulerService, Serializable {

    Context globalContext;
    BufferedReader in = null;
    List<ShopAndOthersDto> listOthersInspectionDto;
    String TAG = "OtherInspectionProcess";
    String serverUrl = "";
    ...
    private void startProcess() {
        boolean sessionInvalid = OtherInspectionTask();
        if(sessionInvalid) {
    ...
    public boolean OtherInspectionTask() {
        boolean unauthorized = false;
        long primaryId = -1;
        try {
            NetworkConnection network = new NetworkConnection(globalContext);
            if (network.isNetworkAvailable()) {
                listOthersInspectionDto = new ArrayList<ShopAndOthersDto>();
                listOthersInspectionDto = FPSDBHelper.getInstance(globalContext).getAllOthersImages();
                for (ShopAndOthersDto othersDto : listOthersInspectionDto) {
                    othersDto.setClientReportId(othersDto.getId());//local db Shop table primary key
                    othersDto.setId(null);
                    String url = serverUrl + "/inspection/app/others";
                    URI website = new URI(url);
                    String otherInspectionData = new Gson().toJson(othersDto);
                    // inserting request into local db
                    primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(otherInspectionData, "OtherInspectionService");
                    StringEntity entity = new StringEntity(otherInspectionData, HTTP.UTF_8);
                    ...
                    if (response != null && response.getStatusLine() != null && response.getStatusLine().getStatusCode() == HttpURLConnection.HTTP_OK) {
                        ...
                        if ((responseData != null) && (!responseData.contains("<html>"))) {
                            ...
                            ShopAndOthersDto shopDto2 = gson.fromJson(responseData, ShopAndOthersDto.class);
                            String messageData = "";
                            String status = "";
                            Log.e(TAG, "response:" + shopDto2.toString());
                            if (shopDto2 != null && shopDto2.getStatusCode() == 0) {
                                Log.e("ShopTableId", "" + shopDto2.getClientReportId());
                                FPSDBHelper.getInstance(globalContext).updateShopAndOtherTable("Others", shopDto2.getClientReportId());
//                        Log.e("OfflineImageShopService Info", "Updating status of Report Image T for transfered " + shopDto.getClientReportId());
                            }
                            else {
                                status = "Failure";
                                try {
                                    messageData = Util.messageSelection(FPSDBHelper.getInstance(globalContext).retrieveLanguageTable(shopDto2.getStatusCode()));
                                    if (messageData == null) {
                ...
                // inserting request into local db
                primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "OtherInspectionService");
                // update response into local db
                if (primaryId != -1) {
        ...
        if (network.isNetworkAvailable()) {
            ...
            try {
                ...
                if(loginDto != null) {
                    ...
                    // inserting request into local db
                    sessionPrimaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(loginDetails, "Session_request_OtherInspectionService");
...
//                        SessionId.getInstance().setFpsId(loginResponse.getUserDetailDto().getFpsStore().getId());
    ...
    private void insertLoginCredentialException() {
        long id = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "Session_request_OtherInspectionService");
