...
public class RemoteLogProcess implements Serializable {

    Context globalContext;
    BufferedReader in = null;
    private GlobalAppState appState;
    String TAG = "RemoteLogProcess";
    String serverUrl = "";

    public void process(Context context, GlobalAppState globalAppState) {
    ...
    private void startProcess() {
        boolean sessionInvalid = RemoteLogTask();
        if(sessionInvalid) {
        ...
        while (!appState.queue.isEmpty()) {
            try {
                ...
                if (network.isNetworkAvailable()) {
                    ...
                    String remoteLogData = new Gson().toJson(appState.queue.dequeue());
                    // inserting request into local db
                    primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(remoteLogData, "RemoteLogService");
                    StringEntity entity = new StringEntity(remoteLogData, HTTP.UTF_8);
                    ...
                    // inserting request into local db
                    primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "RemoteLogService");
                    // update response into local db
                    if (primaryId != -1) {
        ...
        if (network.isNetworkAvailable()) {
            ...
            try {
                ...
                if(loginDto != null) {
                    ...
                    // inserting request into local db
                    sessionPrimaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(loginDetails, "Session_request_RemoteLogService");
...
//                        SessionId.getInstance().setFpsId(loginResponse.getUserDetailDto().getFpsStore().getId());
    ...
    private void insertLoginCredentialException() {
        long id = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "Session_request_RemoteLogService");
