...
public class ShopInspectionProcess implements BaseSchedulerService, Serializable {

    Context globalContext;
    BufferedReader in = null;
    List<ShopAndOthersDto> listShopInspectionDto;
    String TAG = "ShopInspectionProcess";
    String serverUrl = "";
    ...
    private void startProcess() {
        boolean sessionInvalid = ShopInspectionTask();
        if(sessionInvalid) {
    ...
    public boolean ShopInspectionTask() {
        boolean unauthorized = false;
        long primaryId = -1;
        try {
            NetworkConnection network = new NetworkConnection(globalContext);
            if (network.isNetworkAvailable()) {
                listShopInspectionDto = new ArrayList<ShopAndOthersDto>();
                listShopInspectionDto = FPSDBHelper.getInstance(globalContext).getAllShopImages();
                for (ShopAndOthersDto shopDto : listShopInspectionDto) {
                    shopDto.setClientReportId(shopDto.getId());//local db Shop table primary key
                    shopDto.setId(null);
                    String url = serverUrl + "/inspection/app/others";
                    URI website = new URI(url);
                    String shopInspectionData = new Gson().toJson(shopDto);
                    // inserting request into local db
                    primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(shopInspectionData, "ShopInspectionService");
                    StringEntity entity = new StringEntity(shopInspectionData, HTTP.UTF_8);
                    ...
                    if (response != null && response.getStatusLine() != null && response.getStatusLine().getStatusCode() == HttpURLConnection.HTTP_OK) {
                        ...
                        if ((responseData != null) && (!responseData.contains("<html>"))) {
                            ...
                            if (shopDto2 != null && shopDto2.getStatusCode() == 0) {
                                ...
                                FPSDBHelper.getInstance(globalContext).updateShopAndOtherTable("FpsTime", shopDto2.getClientReportId());
//                        Log.e("OfflineImageShopService Info", "Updating status of Report Image T for transfered " + shopDto.getClientReportId());
                ...
                // inserting request into local db
                primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "ShopInspectionService");
                // update response into local db
                if (primaryId != -1) {
        ...
        if (network.isNetworkAvailable()) {
            ...
            try {
                ...
                if(loginDto != null) {
                    ...
                    // inserting request into local db
                    sessionPrimaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(loginDetails, "Session_request_ShopInspectionService");
...
//                        SessionId.getInstance().setFpsId(loginResponse.getUserDetailDto().getFpsStore().getId());
    ...
    private void insertLoginCredentialException() {
        long id = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "Session_request_ShopInspectionService");
