...
public class StockInspectionProcess implements BaseSchedulerService, Serializable {

    Context globalContext;
    BufferedReader in = null;
    List<StockInspectionDto> listStockInspectionDto;
    String TAG = "StockInspectionProcess";
    String serverUrl = "";
    ...
    private void startProcess() {
        boolean sessionInvalid = StockInspectionTask();
        if(sessionInvalid) {
    ...
    public boolean StockInspectionTask() {
        boolean unauthorized = false;
        long primaryId = -1;
        try {
            NetworkConnection network = new NetworkConnection(globalContext);
            if (network.isNetworkAvailable()) {
                listStockInspectionDto = new ArrayList<StockInspectionDto>();
                listStockInspectionDto = FPSDBHelper.getInstance(globalContext).getAllStockImages();
                for (StockInspectionDto stockInspectionDto : listStockInspectionDto) {
                    stockInspectionDto.setClientReportId(stockInspectionDto.getId());//local db stock table primary key
                    stockInspectionDto.setId(null);
                    String url = serverUrl + "/inspection/app/stock";
                    URI website = new URI(url);
                    String stockInspectionData = new Gson().toJson(stockInspectionDto);
                    // inserting request into local db
                    primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(stockInspectionData, "StockInspectionService");
                    StringEntity entity = new StringEntity(stockInspectionData, HTTP.UTF_8);
                    ...
                    if (response != null && response.getStatusLine() != null && response.getStatusLine().getStatusCode() == HttpURLConnection.HTTP_OK) {
                        ...
                        if ((responseData != null) && (!responseData.contains("<html>"))) {
                            ...
                            StockInspectionDto stockInspectionDto2 = gson.fromJson(responseData, StockInspectionDto.class);
                            String messageData = "";
                            String status = "";
                            Log.e(TAG, "response:" + stockInspectionDto2.toString());
                            if (stockInspectionDto2 != null && stockInspectionDto2.getStatusCode() == 0) {
                                status = "Success";
                                Log.e("StockTableId", "" + stockInspectionDto2.getClientReportId());
                                FPSDBHelper.getInstance(globalContext).updateStockTable(stockInspectionDto2.getClientReportId());
                                Log.e(TAG, "Updating status of Report Image T for transfered " + stockInspectionDto2.getClientReportId());
                            } else if (stockInspectionDto2 != null && stockInspectionDto2.getStatusCode() == 8806) {
                                status = "Success";
                                Log.e("StockTableId", "" + stockInspectionDto2.getClientReportId());
                                FPSDBHelper.getInstance(globalContext).updateStockTable(stockInspectionDto2.getClientReportId());
                                Log.e(TAG, "Updating status of Report Image T for transfered " + stockInspectionDto2.getClientReportId());
                                ...
                                status = "Failure";
                                try {
                                    messageData = Util.messageSelection(FPSDBHelper.getInstance(globalContext).retrieveLanguageTable(stockInspectionDto2.getStatusCode()));
                                    if (messageData == null) {
                ...
                // inserting request into local db
                primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "StockInspectionService");
                // update response into local db
                if (primaryId != -1) {
        ...
        if (network.isNetworkAvailable()) {
            ...
            try {
                ...
                if(loginDto != null) {
                    ...
                    // inserting request into local db
                    sessionPrimaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(loginDetails, "Session_request_StockInspectionService");
...
//                        SessionId.getInstance().setFpsId(loginResponse.getUserDetailDto().getFpsStore().getId());
    ...
    private void insertLoginCredentialException() {
        long id = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "Session_request_StockInspectionService");
