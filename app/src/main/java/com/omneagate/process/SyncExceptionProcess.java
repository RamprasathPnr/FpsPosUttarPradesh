...
public class SyncExceptionProcess implements BaseSchedulerService, Serializable {

    Context globalContext;
    BufferedReader in = null;
    List<POSSyncExceptionDto> posSyncExceptionDtoList;
    String localId;
    String TAG = "SyncExceptionProcess";
    String serverUrl = "";
    ...
    private void startProcess() {
        boolean sessionInvalid = SyncExceptionTask();
        if(sessionInvalid) {
    ...
    public boolean SyncExceptionTask() {
        boolean unauthorized = false;
        long primaryId = -1;
        try {
            posSyncExceptionDtoList = new ArrayList<>();
            posSyncExceptionDtoList = FPSDBHelper.getInstance(globalContext).getAllSyncExcData();
            NetworkConnection network = new NetworkConnection(globalContext);
            if (network.isNetworkAvailable()) {
                for (POSSyncExceptionDto posSyncExceptionDto : posSyncExceptionDtoList) {
                    localId = "" + posSyncExceptionDto.getLocalId();
                    String url = serverUrl + "/possyncexception/addPosSyncException";
                    URI website = new URI(url);
                    String syncExceptionData = new Gson().toJson(posSyncExceptionDto);
                    // inserting request into local db
                    primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(syncExceptionData, "SyncExceptionService");
                    StringEntity entity = new StringEntity(syncExceptionData, HTTP.UTF_8);
                    ...
                    if (response != null && response.getStatusLine() != null && response.getStatusLine().getStatusCode() == HttpURLConnection.HTTP_OK) {
                        ...
                        if ((responseData != null) && (!responseData.contains("<html>"))) {
                            ...
                            POSSyncExceptionDto posSyncExceptionDto1 = gson.fromJson(responseData, POSSyncExceptionDto.class);
                            String messageData = "";
                            String status = "";
                            Log.e(TAG, "response:" + posSyncExceptionDto1.toString());
                            if ((posSyncExceptionDto1 != null) && (posSyncExceptionDto1.getStatusCode() == 0)) {
                                status = "Success";
                                FPSDBHelper.getInstance(globalContext).updateSyncExcData(posSyncExceptionDto1, localId);
                            }
                            else {
                                status = "Failure";
                                try {
                                    messageData = Util.messageSelection(FPSDBHelper.getInstance(globalContext).retrieveLanguageTable(posSyncExceptionDto1.getStatusCode()));
                                    if (messageData == null) {
                ...
                // inserting request into local db
                primaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "SyncExceptionService");
                // update response into local db
                if (primaryId != -1) {
        ...
        if (network.isNetworkAvailable()) {
            ...
            try {
                ...
                if(loginDto != null) {
                    ...
                    // inserting request into local db
                    sessionPrimaryId = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory(loginDetails, "Session_request_SyncExceptionService");
...
//                        SessionId.getInstance().setFpsId(loginResponse.getUserDetailDto().getFpsStore().getId());
    ...
    private void insertLoginCredentialException() {
        long id = FPSDBHelper.getInstance(globalContext).insertBackgroundProcessHistory("", "Session_request_SyncExceptionService");
